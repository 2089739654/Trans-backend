<script setup>
    ...
    const fetchText = watch(
        () => route.params.fileId,
            ...
            console.log(newId)
            try { // 根据ID调用API获取文件内容
            const response2 = await fetch(`/documents/${newId}.json`)//(`/api/docs/${newId}`);
            // 关键校验：状态码和内容类型
    ...
    //44444
    // 示例导出数据（整段文本）
    const fullTranslation = ref(`这里是一段完整的翻译结果文本示例...Vue3的组合式API提供了更强大的逻辑封装能力，通过setup函数可以更好地组织代码。相较于Options API，Composition API在复杂组件开发中表现出更好的可维护性。同时，Vue3对TypeScript的支持也更加完善，类型推断能力显著提升...`)
    // 当前句子总数
    ...
    //333333
    // 示例数据
    const systemPrompt = ref(`系统提示：建议注意专业术语一致性...当前项目术语库匹配率：92%`)
...
<template>
    ...
    <div class="section original-text">
        <h3>原文内容</h3>
        <div class="content-box"> {{ textText?.fullText[currentIndex]?.originalText}} </div>
    </div>

    <!-- 初译结果 -->
    <div class="section translation-result">
        <h3>初译结果</h3>
        <div class="content-box">{{ textText?.fullText[currentIndex]?.baseText }}</div>
    </div>
    ...
    <div class="fulltext-scroll">
        ...
        原文
        <div class="translation-content" v-for="(item, index) in textText?.fullText" :key="index">
            <span :class="{ 'highlighted-text': index==currentIndex }"> {{ index + 1 }},{{ item.originalText }} </span>
        </div>
        <!-- </pre> -->
        <!-- <pre class="translation-content">
    ...
    <div class="fulltext-scroll">
        ...
        <!-- <pre class="translation-content"> -->
        <div class="translation-content" v-for="(item, index) in textText?.fullText" :key="index">
            <span :class="{ 'highlighted-text': index==currentIndex }"> {{ index + 1 }},{{ item.finalText }} </span>
        </div>
    </div>
    <!-- 底部固定导出按钮 -->
